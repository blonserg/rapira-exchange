<template>
  <div id="exchange-container" class="d-flex flex-column h-100">
    <el-skeleton :rows="20" animated loading class="vh-100" v-if="isLoading" />
...
export default defineComponent({
  ...
  setup() {
    ...
    const getTradingPairs = async () => {
      ...
      ApiService.post(Api.market.symbolThumb, formData)
        .then((response) => {
    ...
    watch(symbolBase, () => {
      if (symbolBaseHistory.value) {
        socket.emit("UNSUBSCRIBE", [
          `spot@public.depth@${symbolBaseHistory.value}_${symbolRateHistory.value}`,
    ...
    const MobileLayout = defineAsyncComponent(
      () => import("../layouts/exchange-layout/ExchangeMobile.vue")
    );
    const DesktopLayoutSmall = defineAsyncComponent(
      () => import("../layouts/exchange-layout/ExchangeDesktopSmall.vue")
    );
    const DesktopLayoutMedium = defineAsyncComponent(
      () => import("../layouts/exchange-layout/ExchangeDesktopMedium.vue")
    );
    const DesktopLayoutLarge = defineAsyncComponent(
      () => import("../layouts/exchange-layout/ExchangeDesktopLarge.vue")
    );
