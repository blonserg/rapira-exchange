      ...
      <div class="card-px d-flex flex-column align-items-start my-10">
        ...
        <section class="mw-400px w-100">
          ...
          <div class="d-flex mb-6">
            ...
            <span class="fw-bold fs-6">{{ $t('withdraw.chooseCurrency') }}</span>
          </div>
          <div class="mb-8">
            <Multiselect class="mw-400px multiselect-withdraw" v-model="currentCoinUnit" v-bind="dataUnit" @change="changeCoin">
        ...
        <section class="mw-400px w-100">
          ...
          <div class="d-flex mb-5">
            ...
            <span class="fw-bold fs-6">{{ $t('withdraw.optionWithdraw') }} </span>
          </div>
          <article class="mb-8" v-if="currentCoinUnit">
            <div v-if="isFiat">
              <div class="border border-dashed rounded mw-400px py-3 px-4 mb-3 cursor-pointer"
                   :class="[currentMethodDeposit === paymentsType.paymentName ? 'border-primary bg-light-primary': 'border-gray-300', paymentsType.withdrawStatus !== 2 ? 'cursor-na' : 'cursor-pointer']"
                   @click="paymentsType.withdrawStatus === 2 ? changeCurrentMethodDeposit(paymentsType.paymentName) : ''"
                ...
                <div class="d-flex justify-content-between align-items-center">
                  <div class="d-flex">
                    ...
                    <div class="d-flex flex-column">
                      <div class="fw-bold fs-3 d-flex gap-3 align-items-center">
                        ...
                        <span class="badge badge-light-danger badge-square badge-sm px-2 py-0"
                              v-if="paymentsType.withdrawStatus !== 2">Приостановлен</span>
                      </div>
                  ...
                  <span class="badge badge-square badge-light-primary"
                        :class="[currentMethodDeposit === paymentsType.name ? 'badge-primary': 'badge-light-primary']"
                  >2%</span>
                </div>
              </div>
            </div>
            <div v-if="!isFiat">
              <div class="border border-dashed rounded mw-400px w-100 py-3 px-4 me-6 mb-3 cursor-pointer"
                   :class="[currentMethodDeposit === paymentsType.name ? 'border-primary bg-light-primary': 'border-gray-300']"
                   v-for="paymentsType in paymentsTypeCrypto"
                   @click="paymentsType['isWithdrawable'] ? changeCurrentMethodDeposit(paymentsType.name) : ''">
                <div class="d-flex justify-content-between align-items-center">
                  <div class="d-flex">
                    <div class="me-4 position-relative">
                      <div class="symbol symbol-50px">
                        <img
                            :src="getAssetPath(`media/crypto/${paymentsType['coinName']}.svg`)"
                    ...
                    <div class="d-flex flex-column">
                      <div class="fw-bold fs-3 d-flex gap-3 align-items-center">
                        ...
                        <span class="badge badge-light-danger badge-square badge-sm px-2 py-0"
                              v-if="!paymentsType['isWithdrawable']">Приостановлен</span>
                      </div>
                      <div class="fw-bold text-gray-600">
                        {{ $t('withdraw.paymentType1') }}
                      </div>
        ...
        <section class="mw-400px w-100">
          ...
          <div class="d-flex mb-6">
            ...
            <span class="fw-bold fs-6">{{ $t('withdraw.detailsWithdraw') }}</span>
          </div>
          <!-- Содержимое блока -->
          <article v-if="currentMethodDeposit">
            <!-- Блок Криптовалюты -->
            <div class="mw-400px w-100 mb-5" v-if="!needVerification && isCrypto">
              <!-- Блок комиссии и минимальной суммы -->
              <div class="mb-5 mw-400px ">
                <p class="text-gray-600 fs-6 fw-semibold mb-2">{{ $t('withdraw.minSumWith') }}
                  {{ currentNetwork["min"] }} {{ currentCoinUnit }}</p>
                <p class="text-gray-600 fs-6 fw-semibold mb-1">{{ $t('withdraw.notice') }}</p>
              </div>
              <!-- Блок остаток суточного лимита -->
              <div v-if="!isInfinityCryptoWithdraw"
                   class="border border-dashed border-gray-400 rounded bg-light mw-400px w-100 mb-5">
                <div class="d-flex justify-content-between p-7 align-items-center">
                  <div class="d-flex align-items-center">
                    ...
                    >
                      ...
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.limitCrypto') }}
                        {{ limitCryptoWithdraw }} USDT</p>
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.timeRefreshLimit') }}</p>
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.descriptionOperation') }}</p>
                      <template #reference>
                        <span class="d-flex align-items-center ms-1 cursor-pointer">
              ...
              <div class="d-flex flex-column mb-4">
                <div class="mb-4">
                  ...
                  >
                    <span class="d-flex justify-content-between w-100">
                      <span class="fw-bold fs-6">{{ $t('withdraw.addressWithdraw') }}</span>
                      <router-link :to="{ name: 'withdrawAddress'}">{{ $t('withdraw.savedAddresses') }}</router-link>
                    </span>
                  </label>
                  <el-autocomplete
                ...
                <div class="mb-4">
                  <label class="d-flex text-gray-600 justify-content-between align-items-center form-label mb-1">
                    ...
                    <span>
                      <span class="fw-bold fs-7"><span class="fs-6 fw-normal">{{ $t('withdraw.balance') }} </span> {{
                        numberFormatting(balanceCoin["balance"], balanceCoin["coinScale"])
                        }} {{ currentCoinUnit }}</span>
                    </span>
                  </label>
                  <div class="position-relative">
                    <input v-model="amount" type="number" :min="currentNetwork['min']"
                           :class="{ 'is-invalid': amountCryptoError }" @keydown.enter="handleCrypto"
                           style="background-image:none" class="form-control" placeholder=""/>
                    <div class="position-absolute translate-middle-y top-50 end-0 me-5 fw-bold fs-5">{{
                        currentCoinUnit
                      }}
                    </div>
                  </div>
                ...
                <div class="d-flex justify-content-between text-gray-600 align-items-center mb-3"><span
                    class="fs-6">{{ $t('withdraw.commission') }}</span><span
                    class="fw-semibold fs-6">{{ currentFee }}</span></div>
                <div class="d-flex justify-content-between text-gray-600 align-items-center"><span
                    class="fs-6">{{ $t('withdraw.recievedSum') }}</span>
                  <span class="fw-semibold fs-3">{{ currentAmount }} {{ currentCoinUnit }}</span></div>
              </div>
              <button class="btn btn-primary w-100 fs-6" @click="handleCrypto">{{ $t('button.withdraw') }}</button>
            </div>
            <!-- Фиат, наличные -->
            <div class="mw-400px w-100" v-if="!needVerification && isCash">
              <!-- Описание -->
              <p class="text-gray-600 fs-7 fw-semibold mb-0">{{ $t('withdraw.commission') }} 0%.</p>
              <p class="text-gray-600 fs-7 fw-semibold mb-0">{{ $t('withdraw.minSum') }}
                {{ numberFormatting(minAmountFiat, 2) }}
                ₽.</p>
              <p class="text-gray-600 fs-7 fw-semibold">{{ $t('withdraw.stepSum') }}</p>
              <!-- Блок остаток суточного лимита -->
              <div class="border border-dashed border-gray-400 rounded bg-light w-400px mb-5"
                ...
                <div class="d-flex justify-content-between p-7">
                  <div class="d-flex align-items-center">
                    ...
                    >
                      ...
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.limitCash') }}
                        {{ limitCashWithdraw }} USDT</p>
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.timeRefreshLimit') }}</p>
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.descriptionOperation') }}</p>
                      <template #reference>
                        <span class="d-flex align-items-center ms-1 cursor-pointer">
              ...
              <div class="d-flex flex-column mb-7">
                <label class="d-flex text-gray-600 justify-content-between align-items-center form-label mb-1">
                  <span class="fw-bold fs-6">{{ $t('withdraw.total') }}</span>
                  <span>
                  <span class="fw-bold fs-7"><span class="fs-6 fw-normal">{{
                      $t('withdraw.balance')
                    }}</span> {{ numberFormatting(balanceCoin["balance"], 2) }} RUB</span>
                </span>
                </label>
                <div class="position-relative">
                  <input v-model="amount" type="number" :min="minAmountFiat" :class="{'is-invalid': amountError}"
                         style="background-image:none" class="form-control" placeholder="" @keydown.enter="handleCash"/>
                  <div class="position-absolute translate-middle-y top-50 end-0 me-5 fw-bold fs-5">RUB</div>
                </div>
              ...
              <div class="d-flex flex-column mb-4">
                <div v-if="!isSuperUser">
                  <div class="form-floating mb-7">
                    <input type="email" :class="{'is-invalid': fullNameError}"
                           class="form-control text-gray-600 fs-7 fw-bold" id="floatingInputRCode"
                           v-model="fullName" placeholder="Иванов Иван Иванович" @keydown.enter="handleCash" disabled/>
                    <label for="floatingInputRCode" class="text-gray-600 fs-7 fw-semibold">ФИО</label>
                  </div>
              ...
              <p class="text-gray-600 fs-6 fw-semibold mb-5">
                {{ $t('withdraw.notificationAccess') }} <span class="text-primary">Telegram</span>.
              </p>
              <!-- Описание -->
              <div class="d-flex mb-5">
              <span class="d-flex mt-1 me-3">
                ...
                <p class="text-gray-600 fs-6 fw-semibold mb-0">
                  {{ $t('withdraw.afterCreate') }} <span class="text-primary">Telegram {{ $t('withdraw.bot') }}</span>
                  {{ $t('withdraw.forRecieve') }}
                </p>
              </div>
              <button class="btn btn-primary w-100 fs-6" @click="handleCash">{{ $t('button.withdraw') }}</button>
            </div>
            <!-- Фиат, карты -->
            <div class="mw-400px w-100" v-if="!needVerification && isBankCard">
              <!-- Описание -->
              <p class="text-gray-600 fs-6 fw-semibold mb-0">{{ $t('withdraw.commission') }} 2%.</p>
              <p class="text-gray-600 fs-6 fw-semibold">{{ $t('withdraw.minSum') }}
                {{ numberFormatting(minAmountFiat, 2) }} ₽.
                {{ $t('withdraw.maxSum') }} {{ numberFormatting(maxAmountFiat, 2) }} ₽.</p>
              <!-- Блок остаток суточного лимита -->
              <div class="border border-dashed border-gray-400 rounded bg-light mw-400px mb-5"
                ...
                <div class="d-flex justify-content-between p-7">
                  <div class="d-flex align-items-center">
                    ...
                    >
                      ...
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.limitFiat') }}
                        {{ limitFiatWithdraw }} USDT</p>
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.timeRefreshLimit') }}</p>
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.descriptionOperation') }}</p>
                      <template #reference>
                        <span class="d-flex align-items-center ms-1 cursor-pointer">
              ...
              <div class="d-flex flex-column mb-7">
                ...
                <div>
                  <label class="text-gray-600  mb-1"><span class="fw-bold fs-7">{{
                      $t('withdraw.numberCard')
                    }}</span></label>
                  <input type="text" v-model="cardNumber" class="form-control" :class="{'is-invalid': cardNumberError}"
                         style="background-image:none" placeholder="0000 0000 0000 0000" @keydown.enter="handleFiat">
                  <span class="text-danger">{{ cardNumberError }}</span>
                </div>
              </div>
              <button class="btn btn-primary w-100 fs-6" @click="handleFiat()">{{ $t('button.withdraw') }}</button>
            </div>
            <!-- rCode -->
            <div class="mw-400px w-100" v-if="!needVerification && isRCode">
              <!-- Описание rCode -->
              <p class="text-gray-600 fs-6 fw-semibold">{{ $t('withdraw.generateCode') }}</p>
              <!-- Блок остаток суточного лимита -->
              <div class="border border-dashed border-gray-400 rounded bg-light mw-400px mb-5" v-if="!isInfinityRCode">
                <div class="d-flex justify-content-between p-7">
                  <div class="d-flex align-items-center">
                    <p class="text-gray-500 fw-bold fs-6 mb-0 mw-400px ">
                      {{ $t('withdraw.dailyLimit') }}
                    ...
                    >
                      ...
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.limitRcode') }}
                        {{ limitRCodeWithdraw }} USDT</p>
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.timeRefreshLimit') }}</p>
                      <p class="fs-8 mb-1 lh-1">{{ $t('withdraw.descriptionOperation') }}</p>
                      <template #reference>
                        <span class="d-flex align-items-center ms-1 cursor-pointer">
              ...
              <div class="mb-4">
                ...
                <div class="position-relative">
                  <input v-model="amount" type="number" min="0" :class="{ 'is-invalid': amountRCodeError }"
                         style="background-image:none" class="form-control" placeholder=""
              ...
              <button class="btn btn-primary w-100 fs-6" @click="handleRCode()">{{ $t('button.withdraw') }}</button>
            </div>
            <!-- Блок верификации -->
            <div v-if="needVerification" class="bg-light-primary border border-primary rounded w-400px p-9 me-6 mb-4">
              <div class="d-flex flex-column justify-content-center">
                <h3 class="fw-bold mb-5">{{ $t('withdraw.enterData') }}</h3>
                <p class="text-gray-600 fs-6 fw-semibold">{{ $t('withdraw.forWork') }}</p>
                <div class="mb-5">
                  <div class="d-flex align-items-center">
                <span class="d-flex align-items-center me-3">
                  <i class="ki-outline ki-time fs-3"></i>
                    ...
                    <p class="text-gray-600 mb-1">
                      <span class="fw-bold">{{ $t('withdraw.checkTime') }}</span>
                  ...
                  <div class="d-flex align-items-center">
                    ...
                    <p class="text-gray-600 mb-0">
                      <span class="fw-bold">{{ $t('withdraw.documents') }}</span>
                      {{ $t('withdraw.enoughFoto') }}
                    </p>
                  </div>
                </div>
                <button class="btn btn-primary w-150px fs-7" @click="">{{ $t('withdraw.specifyData') }}</button>
              </div>
            </div>
          </article>
        </section>
...
export default defineComponent({
  ...
  methods: {
    ...
    handleSubmitRCode(code) {
      ...
      ApiService.post(Api.uc.withdrawCodeApplyCode, formData).then(response => {
        if (response.data.code === 200 || response.data.code === 0) {
          ...
          Swal.fire({
                ...
                customClass: {
                  ...
                  htmlContainer: "mx-0",
                  input: "form-control form-control-lg form-control-solid w-auto",
                  actions: "flex-row-reverse justify-content-start align-items-center w-100 m-0 mt-3 gap-5 pt-0 border-light",
                  confirmButton: "btn btn-lg btn-primary mw-100 rounded px-4 py-3 m-0",
                  cancelButton: "btn btn-light rounded px-4 py-3 m-0",
                }
              },
          )
          this.withdrawStore.setRcode(`${response.data.message} ${Date.now()}`);
        } else {
    ...
    handleCash() {
      if (this.amount < this.minAmountFiat) this.amountError = "Сумма меньше минимально возможной";
    ...
    handleFiat() {
      if (!this.cardNumber) this.cardNumberError = "Введите валидный номер карты";
      if (this.amount < this.minAmountFiat) this.amountError = "Сумма меньше минимально возможной";
      if (this.cardNumberError || this.amountError) return;
      this.modal2FA("handleSubmitFiat");
    },
    handleCrypto() {
      if (this.amount < this.minAmountCrypto) this.amountCryptoError = "Сумма меньше минимально возможной";
      if (!this.currentAddress) this.currentAddressError = "Введите адрес вывода";
      if (this.amountCryptoError || this.currentAddressError) return;
      this.modal2FA("handleSubmitCrypto")
    },
    handleSubmitCrypto(code) {
      let formData = new FormData();
      formData.append("coinName", this.currentUnit(this.currentCoinUnit)["name"])
      ...
      ApiService.post(Api.uc.withdrawCrypto, formData).then(response => {
        if (response.data.code === 0 || response.data.code === 200) {
          ElMessage.success(response.data.message);
          this.withdrawStore.setCrypto(`${response.data.message} ${Date.now()}`);
    ...
    handleSubmitFiat(code) {
      ...
      formData.append("paymentTypeId", this.currentPaymentTypeFiat(this.currentMethodDeposit)["id"]);
      formData.append("code", code);
      formData.append("fullName", this.fullName);
      ApiService.post(Api.uc.legalWalletWithdraw, formData).then(response => {
        if (response.data.code === 0 || response.data.code === 200) {
          this.amount = 0;
          this.cardNumber = "";
          ElMessage.success(response.data.message);
          this.withdrawStore.setFiat(`${response.data.message} ${Date.now()}`);
          this.successOrErrorSubmit(`Код заявки: ${response.data.data}`, "Код всегда можно посмотреть в истории Фиата", "success")
        } else {
          this.successOrErrorSubmit('Ошибка вывода', response.data.message, 'error');
        }
      }).catch(response => {
        this.successOrErrorSubmit('Ошибка вывода', response.data.message, 'error');
      });
    ...
    getPassesNames() {
      axios.get(Api.uc.passesName).then(response => {
    ...
    getStatus() {
      axios.get(Api.uc.businessStatus).then(response => {
        if (response.data.code === 0 || response.data.code === 200) {
    ...
    getLimits() {
      ApiService.post(Api.uc.limits, "").then(response => {
        if (response.data.code === 0 || response.data.code === 200) {
          this.limit = response.data.data.find(item => item.level === this.memberLevel);
          ApiService.post(Api.uc.myLimit, "").then(response => {
            if (response.data.code === 0 || response.data.code === 200) {
              this.myLimit = response.data.data;
              this.limitRCodeWithdraw = this.numberFormatting(Number(this.limit.rcodeCreate) - Number(this.myLimit.rcodeCreate), 2); // Лимиты rCode
              this.limitFiatWithdraw = this.numberFormatting(Number(this.limit.fiatWithdraw) - Number(this.myLimit.fiatWithdraw), 2); // Лимиты вывода на карту
              this.limitCashWithdraw = this.numberFormatting(Number(this.limit.cashWithdraw) - Number(this.myLimit.cashWithdraw), 2); // Лимиты вывода наличкой
              this.limitCryptoWithdraw = this.numberFormatting(Number(this.limit.cryptoWithdraw) - Number(this.myLimit.cryptoWithdraw), 2); // Лимиты вывода криптовалюты
              this.isInfinityRCode = this.limit.rcodeCreate === -1; // Есть ли лимиты на rCode
              this.isInfinityCryptoWithdraw = this.limit.cryptoWithdraw === -1; // Есть ли лимты на вывод крипты
    ...
    getAddress() {
      axios.get(Api.uc.addressWithdraw)
          .then(response => {
            if (response.data.code === 0 || response.data.code === 200) {
    ...
    getPaymentsTypeFiat() {
      axios.post(Api.uc.paymentTypes + `?type=1`).then(response => {
        if (response.data.code === 0 || response.data.code === 200) {
    ...
    getDataUnit() {
      ApiService.post(Api.uc.coinFull, "").then(response => {
        if (response.data.code === 0 || response.data.code === 200) {
          this.dataUnit.options = response.data.data.coin.filter(item => item["isFiat"]).map(coin => coin.unit);
          this.dataUnit.options = [...this.dataUnit.options, ...response.data.data.coin.filter(item => !item["isFiat"]).map(coin => coin.unit)]
          ...
          this.networks.forEach((itemNetworks, indexNetworks) => {
            this.wallet.forEach(itemWallet => {
              if (itemNetworks.coinName === itemWallet["name"]) {
                this.networks[indexNetworks]["isWithdrawable"] = itemWallet["isWithdrawable"];
              }
            })
          })
        } else ElMessage.error(response.data.message);
  ...
  computed: {
    ...
    currentAmount() {
      if (this.currentMethodDeposit && this.isCrypto && this.amount >= this.currentFee) {
        return this.numberFormatting(this.amount - this.currentFee, this.currentNetwork["scale"])
      }
      return 0;
    },
    currentFee() {
    ...
    balanceCoin() {
      if (this.currentMethodDeposit) {
        return this.wallet.find(item => item.unit === this.currentCoinUnit) || {balance: 0, coinScale: 2}
      }
    },
    currentNetwork() {
      if (this.currentMethodDeposit && this.isCrypto) {
        return this.networks.find(item => item.name === this.currentMethodDeposit);
      }
    },
    userLevelText() {
    ...
    maxAmountFiat() {
      if (this.currentMethodDeposit) {
        return this.paymentsTypeFiat.filter(item => item.paymentName === this.currentMethodDeposit)[0]["withdrawMaxAmount"];
      }
    },
    minAmountFiat() {
      if (this.currentMethodDeposit) {
        return this.paymentsTypeFiat.filter(item => item.paymentName === this.currentMethodDeposit)[0]["withdrawMinAmount"];
      }
    },
    minAmountCrypto() {
      if (this.currentMethodDeposit && this.isCrypto) {
    ...
    isCash() {
      if (this.currentMethodDeposit && this.isFiat) {
        return this.paymentsTypeFiat.filter(item => item.paymentName === this.currentMethodDeposit)[0]["category"] === 2;
      }
      return false;
    },
    isSuperUser() {
    ...
    isFiat() {
      if (this.currentCoinUnit) {
        return this.dataUnit.data.filter(item => item["unit"] === this.currentCoinUnit)[0]["isFiat"];
      }
      return false;
    },
    isCurrent() {
