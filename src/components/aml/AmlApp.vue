  ...
  <div class="card mb-3">
    <div class="card-body p-0">
      <div class="card-px d-flex flex-column align-items-start my-10">
        <h4 v-if="checkedNumber.length < 65" class="fs-5 mb-5">
          Проверка адреса
        </h4>
        ...
        <div class="d-flex justify-content-between w-100 mb-3">
          <div class="d-flex align-items-center position-relative flex-grow-1">
            ...
            <input
              type="text"
              class="form-control fs-7 ps-12 py-3"
              v-model="checkedNumber"
              placeholder="Введите адрес или ID транзакции"
            />
          </div>
          <Multiselect
            class="w-150px mx-4"
        ...
        <div v-if="checkedNumber.length > 64" class="d-flex mt-9">
          <div class="form-check form-check-custom form-check-solid me-6">
            ...
            />
            <label
              class="form-check-label fw-bold text-gray-600 fs-7"
...
export default defineComponent({
  ...
  setup() {
    ...
    const checkOperation = async () => {
      ...
      ApiService.post(Api.uc.amlCheck, formData)
        .then((response) => {
    ...
    watch(checkedNumber, () => {
      checkedNumber.value.length > 64
        ? (directionCheck.value = "0")
        : (directionCheck.value = "2");
    });
