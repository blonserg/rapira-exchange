<template>
  ...
  >
    <div class="card">
      <div
        class="card-header border-0 pt-0 d-flex justify-space-between align-items-center"
      >
            ...
            <div class="d-flex flex-column flex-lg-row justify-content-between">
              <div class="w-100 pe-lg-4 border-end">
                ...
                <div class="position-relative">
                  ...
                  />
                  <div
                    class="position-absolute translate-middle-y top-50 end-0 me-5 fw-bold fs-5 text-gray-600"
                  >
                    {{ firstPair }}
                  </div>
                </div>
                ...
                <div class="d-flex justify-content-between mt-3">
                  ...
                  <div class="fs-4 fw-bold">
                    <span v-if="buyComissionMarket">{{
                      numberFormatting(buyComissionMarket, 2)
                    }}</span>
                    <span v-else>0</span>
                    {{ secondPair }}
                    <span> / 0.2%</span>
                ...
                <div v-if="!isAuthenticated" class="mt-3">
                  <router-link class="btn btn-primary w-100 fs-5" to="/auth">
                    <span>{{ $t("exchange.login") }}</span>
                    {{ $t("exchange.or") }}
                    <span>{{ $t("exchange.register") }}</span>
                  </router-link>
                </div>
            ...
            <div class="d-flex flex-column flex-lg-row justify-content-between">
              <div class="w-100 pe-lg-4 border-end">
                ...
                <div class="d-flex justify-content-between mt-3">
                  ...
                  <div class="fs-4 fw-bold">
                    <span v-if="buyComission">{{
                      numberFormatting(buyComission, 2)
                    }}</span>
                    <span v-else>0</span>
                    {{ secondPair }}
                    <span> / 0.2%</span>
            ...
            <div class="d-flex flex-column flex-lg-row justify-content-between">
              <div class="w-100 ps-lg-4">
                ...
                <div class="d-flex justify-content-between mt-3">
                  ...
                  <div class="fs-4 fw-bold">
                    <span v-if="sellComissionMarket">{{
                      numberFormatting(sellComissionMarket, 2)
                    }}</span>
                    <span v-else>0</span>
                    {{ secondPair }}
                    <span> / 0.2%</span>
          ...
          >
            <div class="d-flex flex-column flex-lg-row justify-content-between">
              <div class="w-100 ps-lg-4">
                ...
                <div class="d-flex justify-content-between mt-3">
                  <div class="fs-4 fw-bold">
                    {{ $t("exchange.commisssion") }}
                  </div>
                  <div class="fs-4 fw-bold">
                    <span v-if="sellComission">{{
                      numberFormatting(sellComission, 2)
                    }}</span>
                    <span v-else>0</span>
                    {{ secondPair }}
                    <span> / 0.2%</span>
...
export default defineComponent({
  ...
  setup() {
    ...
    const buyComissionMarket = computed(
      () => (buyTotalMarket.value / 100) * 0.2
    );
    const sellComissionMarket = computed(
      () => (sellTotalMarket.value / 100) * 0.2
    );
      ...
      axios
        ...
        .then((response) => {
          if (response.data.code === 0 || response.data.code === 200) {
            ...
            limitOfFirstCurrency.value = res.find(
              (item) => item.unit === firstPair.value
            )?.balance;
            limitOfSecondCurrency.value = res.find(
              (item) => item.unit === secondPair.value
            )?.balance;
    ...
    const actionDeal = (symbol, price, amount, direction, type) => {
      ...
      ApiService.post(Api.exchange.orderAdd, formData)
        .then((response) => {
          if (response.data.code === 0 || response.data.code === 200) {
    ...
    const getMarketPrice = async (orderbook, trueDir, volume, input) => {
      ...
      if (volume_left === 0) {
        ...
        result_price = isNormalVolume
          ? turnover / (volume - volume_left)
          : (volume - volume_left) / turnover;
      }
  ...
  methods: {
    ...
    changeBuyAmountMarket() {
      this.buyTotalMarket =
        this.buyAmountMarket * Number(this.sellPriceHistory);
    },

    changeSellAmountMarket() {
      this.sellTotalMarket =
        this.sellAmountMarket * Number(this.buyPriceHistory);
    },
